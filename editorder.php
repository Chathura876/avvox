...
<?php
$permissions = array("admin", "dealer", "shop");
...
<form method="post" id= "udaraform" autocomplete="off" action="process/editorder.inc.php">
	...
	if ($result = $mysqli->query($query)) {
		while ($orderitem = $result->fetch_object()) {
			...
			if($orderitem->amount > 0){
				...
				<td>
				<input type='number' min='{$orderitem->amountissued}' id='' name='products[{$orderitem->id}][]' value='$totalamountforthisproduct' data-validation-engine='validate[required,custom[integer], min[{$orderitem->amountissued}],  funcCall[validateamount]]' /> {$orderitem->amountissued} already issued
				</td>
