...
<?php
$permissions = array("admin","director","operator","accountant");
...
<div class="col-12">
  <h5>Manage non-approved Jobs</h5>
 ...
 <script type="text/javascript">
    ...
    $(document).ready(function () {
      ...
      $('#PeopleTableContainer').jtable({
        ...
        actions: {
          listAction: 'process/nonapprovedjobs.inc.php?action=list'
        },
      ...
      $('#PeopleTableContainer').jtable('load');
          ...
          function approvejob(button){
            bootbox.confirm("Approve this job?", function(result){ 
              if(result){
                $.ajax({
                  type: 'POST',
                  async: false,
                  url: 'process/approvejob.inc.php',
                  data: { "jobid": $(button).val()},
                  dataType: 'html',
                  success: function (data) {
                    if(data == 'approved'){
                      $(button).html("Approved");
                      $(button).prop("disabled", true);
                      //bootbox.alert("Job Approved!", function(){ 
