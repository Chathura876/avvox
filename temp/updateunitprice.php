...
$query = "SELECT * FROM orderitems WHERE unitprice=0.00";
if ($result = $mysqli->query($query)) {
	while ($objectname = $result->fetch_object()) {
	     //echo $objectname->Name;
	     //echo "HI<br>";

		$dealerquery = "SELECT dealerid FROM orders WHERE orderid={$objectname->orderid} LIMIT 1";
		$thisdealer = 0;
		if ($dealerresult = $mysqli->query($dealerquery)) {
			$objectname2 = $dealerresult->fetch_object();
			$thisdealer = $objectname2->dealerid;
		
		}
		$unitprice = getunitprice($thisdealer, $objectname->modelid);
		//echo "$thisdealer, {$objectname->modelid} $unitprice<br>";

		echo $updatequery = "UPDATE orderitems SET unitprice=$unitprice WHERE id={$objectname->id}";
