...
try
{
	...
	if($_GET["action"] == "list")
	{
		...
		if(isset($_POST["Name"]) && strlen($_POST["Name"]) > 0){
			$query .= "  AND CONCAT_WS(job.id, job.contactname, job.contactno, user2.shopname, user.fullname, job.timeadded, job.timecompleted) LIKE '%".$_POST["Name"]."%'";
		...
		while($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
		{
			...
			// <button type='button' value='{$row['id']}' class='btn btn-primary btn-sm my-1 mx-auto' data-toggle='modal' data-target='#jobverifiedmodal'>&nbsp;&nbsp;Verify Job&nbsp;&nbsp;</button>
