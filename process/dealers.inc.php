...
try
{
	...
	if($_GET["action"] == "list")
	{
		...
		LEFT JOIN user as user2 ON user.repid = user2.id WHERE user.usertype='dealer'";

		if($_SESSION['usertype'] !=('admin' || 'operator')){
			$query .= " AND user.repid={$_SESSION['id']}";
		}

		//UDARA
		if(isset($_POST["Name"]) && strlen($_POST["Name"]) > 0){
			$query .= "  AND CONCAT_WS(user.id, user.fullname, user.shopname, user.nic, user.address, user.area, user.phone1, user.phone2, user.area, user.email, user.pricing, user2.fullname) LIKE '%".$_POST["Name"]."%'";
	...
	{ 
		...
		$result = mysqli_query($mysqli, "INSERT INTO `user` (username, password, fullname, shopname, nic, address, area, phone1, phone2,  usertype, addedby, pricing, repid, role_id) VALUES ('$username', '$password', '$fullname', '$shopname', '$nic', '$address', '$area', '$phone1', '$phone2', 'dealer', {$_SESSION['id']}, '$pricing', $repid, 3)");
