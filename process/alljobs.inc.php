...
try
{
	...
	if($_GET["action"] == "list")
	{
		...
		$query = "SELECT job.id AS id, job.warrantyid, contactname, contactno, job.timeadded, job.status, jobtype, user.fullname, user2.shopname, product.model, customer.address FROM job 
		...
		if(isset($_POST["Name"]) && strlen($_POST["Name"]) > 0){
			$query .= "  AND CONCAT_WS(job.id, job.warrantyid, job.contactname, job.contactno, job.timeadded, user.fullname, user2.shopname, product.model) LIKE '%".$_POST["Name"]."%'";
