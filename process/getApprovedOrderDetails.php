...
{	
	if(isset($_POST['dealerid']) && isset($_GET['status'])){
		...
		$datastring = "<h2>Approved Order Details Report - Avvox CRM</h2>";
		...
		// 		";
			...
			while ($item = $result->fetch_object()) {
				...
				if ($result2 = $mysqli->query($query2)) {
				  ...
				  <tr>
				    <th style='width:40%' align='right'>Model</th>
						...
						$descstring .= "
						  <tr>
							...
							if($_GET['status'] == 0 || $_GET['status'] == 1){
								...
								$total = ($log->amount - $log->amountfree) * $log->unitprice;
								$tol = number_format($total,2);
							}else{
								$descstring .= "<td align='right'>{$log->amountissued}</td>";
								$total = ($log->amountissued - $log->amountfree) * $log->unitprice;
								$tol = number_format($total,2);
							}
