...
try
{
	...
	if($_GET["action"] == "list")
	{
		...
		$query = "SELECT id, fullname, nic, address, phone1, phone2, area FROM user WHERE usertype='technician'";

		//UDARA
		if(isset($_POST["Name"]) && strlen($_POST["Name"]) > 0){
			$query .= "  AND CONCAT_WS(fullname, nic, address, phone1, phone2, area) LIKE '%".$_POST["Name"]."%'";
	...
	{ 
		...
		//echo "INSERT INTO `user` (username, password, fullname, nic, address, phone1, phone2, area,  usertype, addedby) VALUES ('$username', '$password', '$fullname', '$nic', '$address', '$phone1', '$phone2', '$area', 'technician', {$_SESSION['id']})";

		$result = mysqli_query($mysqli, "INSERT INTO `user` (username, password, fullname, nic, address, phone1, phone2, area, usertype, addedby, role_id) VALUES ('$username', '$password', '$fullname', '$nic', '$address', '$phone1', '$phone2', '$area', 'technician', {$_SESSION['id']},5)");
