...
try
{
	...
	if($_GET["action"] == "list")
	{
		...
		$query = "SELECT id, fullname, nic, address, phone1, phone2, email, usertype FROM user";

		//UDARA
		if(isset($_POST["Name"]) && strlen($_POST["Name"]) > 0){
			$query .= "  AND CONCAT_WS(fullname, nic, address, phone1, phone2, email, usertype) LIKE '%".$_POST["Name"]."%'";
	...
	{ 
		...
		if($_POST['usertype'] == 'admin'){
			$roleId = 2;
		}else if($_POST['usertype'] == 'dealer'){
			$roleId = 3;
		}else if($_POST['usertype'] == 'salesrep'){
			$roleId = 4;
		}else if($_POST['usertype'] == 'technician'){
			$roleId = 5;
		}else if($_POST['usertype'] == 'storeskeeper'){
			$roleId = 6;
		}else if($_POST['usertype'] == 'accountant'){
			$roleId = 7;
		}else if($_POST['usertype'] == 'director'){
			$roleId = 8;
		}else if($_POST['usertype'] == 'shop'){
			$roleId = 10;
		}else if($_POST['usertype'] == 'operator'){
			$roleId = 11;
		}
		//Insert record into database

		$result = mysqli_query($mysqli, "INSERT INTO `user` (username, password, fullname, nic, address, phone1, phone2, email, usertype, role_id) VALUES ('$username', '$password', '$fullname', '$nic', '$address', '$phone1', '$phone2', '$email', '$usertype','$roleId')");
