	...
	if ($result = $mysqli->query($query)) {
		while ($orderitem = $result->fetch_object()) {
			...
			if($orderitem->amount > 0){
				...
				<td>
				<input type='number' min='0' max='{$orderitem->amount}' id='' name='products[{$orderitem->id}][]' value='{$orderitem->amount}' data-validation-engine='validate[required,custom[integer], funcCall[validateamount]]' />
				</td>

				<input type='hidden' id='model{$orderitem->id}' name='products[{$orderitem->id}][]' value='{$orderitem->modelid}'>

				<td>
				<input type='number' min='0' max='$totalamountforthisproduct' id='' name='products[{$orderitem->id}][]' value='$totalamountforthisproduct' data-validation-engine='validate[required,custom[integer], funcCall[validateamount]]' />
				</td>
