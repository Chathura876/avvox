...
if (isset($_GET["orderid"])) {
	...
	$invoicedata = "SELECT * FROM issue WHERE orderid={$_GET["orderid"]}";
	...
	if ($result = $mysqli->query($query)) {
		...
		$invoiceresult = $mysqli->query($invoicedata);
		$invoice = $invoiceresult->fetch_object();
	}

	$ordertime = date("Y-m-d", $order->ordertime);
    $issuedTime = date("Y-m-d", $order->issuedtime);
...
<tr>
	...
	<td style='font-size:14px; width: 20%; margin-right:0px'>
	<br>
	Invoice Number:<b> {$invoice->invoiceId}</b><br>
	Invoice Date: $issuedTime<br>
	...
	//Check if this order has free items, hide Free column if there aren't any free items
	$isfreeitems = 0;

	$queryfree = "SELECT * FROM orderitems WHERE orderid={$_GET["orderid"]}";
	if ($resultfree = $mysqli->query($queryfree)) {
		while ($objectname = $resultfree->fetch_object()) {
			if ($objectname->amountfree > 0) {
				$isfreeitems = 1;
			}
			//echo "<br>";
		}
...
<tr>
<td style='background-color:none;'><b>Model</b></td>
<td><b>Total Qty</b></td>";

	if ($isfreeitems) {
		$invoicestring .= "<td><b>Free</b></td>";
	}

	$invoicestring .= "<td align='right'><b>Unit Price(Rs)</b></td>
...
WHERE orderid={$_GET["orderid"]}";
	if ($result = $mysqli->query($query)) {
        ...
        */
			...
			$amountforthisitem = $thisproduct->amount + $thisproduct->amountissued-$thisproduct->amountfree;
			$totalamount += $amountforthisitem;

			$invoicestring .= "<tr>
		<td>{$thisproduct->model}</td>
		<td align='right'>$amountforthisitem</td>";
			if ($isfreeitems) {
				$invoicestring .= "<td align='right'>{$thisproduct->amountfree}</td>";
			}
	...
	if ($isfreeitems) {
		$invoicestring .= "
<tr><td colspan='4' align='right'><b>Total:</b></td><td align='right'><b>Rs. $billtotal</b></td></tr>";
	} else {
		$invoicestring .= "
<tr><td><b>Total:</b></td><td align='right'><b>$totalamount</b></td><td></td><td align='right'><b>Rs. $billtotal</b></td></tr>";
	}
	$invoicestring .= "</table>";
...
<table border='1' style='width:40%; float:left'>
  <tr>
    <td style='padding:5px;'><b>Prepared by</b></td>
  </tr>
  <tr>
...
<table border='1' style='width:40%; float:right; clear:bottom'>
  <tr>
    <td style='padding:5px;'><b>Customer Signature & Stamp</b></td>
  </tr>
  <tr>
    <td style='border:0px; padding:5px'><br>Goods Recieved in Good Condition<br><br>Signature: .....................................................<br><br></td>
  </tr>
</table>
...
<div>*Terms & Conditions<br>
<ul>
    <li>Warranty 1year for all the items from date of delivery goods.</li>
    
    <li>Cheques to be drawn in favour of 'AVVOX HOLDINGS (PVT) LTD' and crossed ACCOUNT PAYEE ONLY. </li>
</ul></div>
";
...
<form action="process/generateinvoicepdf.php" method="POST">
	...
	<button type="button" class="btn btn-success ml-3 float-right" onclick='window.print()'>Print</button>
	<button type="submit" class="btn btn-success float-right">PDF</button>
</form>
