...
<form>
  <div class="row">
    ...
    <div class="form-group col-md-6 d-print-none">
      <label for="repid">Sales Rep</label>
      <select class="form-control" id="repid" name="repid">
        <option value='0'>All</option>
        <?php
        $query = "SELECT id, fullname FROM user WHERE usertype IN ('salesrep') ORDER BY fullname ASC";
        if ($result = $mysqli->query($query)) {
          while ($objectname = $result->fetch_object()) {
...
<div class="row">
  <div class="col-6 offset-6 offset-md-9 col-md-3 d-print-none float-right btn-lg btn px-3">
    ...
    <form action="process/generateorderspdf.php" method="POST">
      ...
      <!--<button type="submit" class="btn btn-primary float-right mr-3">Excel</button>-->
    </form>
    <!--<form action="process/generateexcel.php" method="POST">-->
    <!--    <input type="submit" name="export_excel" value="Excel" class="btn btn-primary float-right mr-3">-->
    <!--</form>-->
  ...
  $(document).ready(function() {
    ...
    function getpendingjosbreport() {
      ...
      $.ajax({
        type: 'POST',
        async: false,
        url: 'process/getsalesreport.php?status=1',
        data: {
