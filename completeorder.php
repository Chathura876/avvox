...
if(isset($_GET["orderid"])){
	...
	$statusquery = "SELECT * FROM orders WHERE orderid={$_GET["orderid"]}";
	...
	if($statusresult = $mysqli->query($statusquery)) {
		...
		if($thisorder->issued >1 && $updatetype =='edit' && $_SESSION['usertype'] !=('admin' || 'operator')){//Orders can't edit after issuing stage
			echo "<div class='row'>
			<div class='col-12 mb-3 alert-danger'>
...
<form method="post" id= "udaraform" action="process/completeorder.inc.php">
...
<button class="btn btn-primary" id="complete" name="complete" type="submit">Complete</button>
<button class="btn btn-primary" id="notdelivered" name="notdelivered" type="submit">Not Delivered</button>
</div>
...
<?php if( $_SESSION['usertype'] != ('admin' || 'operator')){//only admins can edit ?>
	$(":input[type=number], button.btn-danger, #model, #additembutton").prop("disabled", true);
 <?php } ?>
...
if ($result = $mysqli->query($query)) {
	...
	while ($orderitem = $result->fetch_object()) {
		$thisitemamount = $orderitem->amount + $orderitem->amountissued - $orderitem->amountfree;
		?>
